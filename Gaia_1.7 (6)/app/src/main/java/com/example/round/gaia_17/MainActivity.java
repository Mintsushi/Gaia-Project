...
public class MainActivity extends AppCompatActivity
    ...
    // 추가부분
    ///////////////////DB//////////////////////////////
    public static goalListDialog.GoalUserInfomation mGoalUserInfomation = new goalListDialog.GoalUserInfomation();
    public static ArrayList<goalListDialog.GoalFormatInformation> mGoalFormatArray = new ArrayList<>();
    public static ArrayList<goalListDialog.GoalInformation> mGoalArray =new ArrayList<>();
    public static ArrayList<Water> mWaterArray =new ArrayList<>();
    // 아이템 임시저장
    public static int[] itemsSave = new int[]{0,0,0};
    // hp
    TextView hpText;
    ...
    public static class HpManager{
        int hpMax;
        int hpPre;
        TextView interimHpText;
        ProgressBar hpProgressBar;
        HpManager(TextView textView, ProgressBar progre){
            interimHpText = textView;
            hpProgressBar = progre;
            this.hpMax = 100;
            this.hpPre = 100;
        }

        public void setHpText() {
            interimHpText.setText("" + getHpMax() + " / " + getHpPre());
            hpProgressBar.setProgress(getHpPre());
            Log.i("interimHpText","" + getHpMax() + " / " + getHpPre());
        }
        public int getHpPre() {
            return hpPre;
        }
        ...
        public void setHpPre(int hpPre) {
            this.hpPre -= hpPre;
        }
    }
    ...
    public void onClick(View view) {
        ...
        mGoalUserInfomation.setGoalNumberArr(9,1);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mGoalFormatArray = db.getGoalFormatInform();
        mWaterArray = db.getWaterInform();
        for(int i =0;i<mGoalFormatArray.size();i++){
            mGoalArray.add(db.getGoalInform(mGoalFormatArray.get(i).getGoalFormatNextDrect()));
        }
        ...
        hpText = (TextView)findViewById(R.id.interimText);
        hpProgressBar = (ProgressBar)findViewById(R.id.HPProgressBar);
        setImageButtonClick();

        hpManager = new HpManager(hpText,hpProgressBar);
        hpManager.setHpText();
        //OverlayServie
    ...
    private void testSource() {
        ...
        plantArray.add(new PlantInfo(0, imageView, "민들레", 0,mWaterArray.get(0)));
    }
    ...
    protected static class PlantInfo implements View.OnTouchListener, View.OnClickListener {
        ...
        public PlantInfo(int id, ImageView plant, String name, int lv, final Water waterEffect) {
            ...
            water.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if(itemsSave[2]>=waterEffect.getWaterNeedWaterNum()) {
                        Toast.makeText(context, "물 주기 성공", Toast.LENGTH_LONG).show();

                        itemsSave[2] = itemsSave[2] - waterEffect.getWaterNeedWaterNum();

                        waterState = 0;
                        water.setVisibility(View.INVISIBLE);
                    }else{
                        Toast.makeText(context, "물 주기 실패 ㅠㅠ", Toast.LENGTH_LONG).show();
        ...
        private android.os.Handler mHandler = new android.os.Handler() {
            public void handleMessage(Message msg) {
                ...
                if(waterState == 0) {
                    mHandler.sendEmptyMessageDelayed(0, waterEffect.waterPeriod * 1000);
                }
                else {

                    mHandler.sendEmptyMessageDelayed(0, waterEffect.waterPenaltyTime * 1000);
                }
                if (waterState == 0) {

                    Toast.makeText(context, "목이말라요 ㅠㅠ", Toast.LENGTH_LONG).show();
                    ...
                    hpManager.setHpPre(waterEffect.getWaterPenalty());
                    hpManager.setHpText();
                }
            }
        };
    ...
    public static void buyNewPlant(int id, int lv, String name, String imagePath) {
        ...
        plantArray.add(new PlantInfo(id, plant, name, lv, mWaterArray.get(id)));
    }
    ...
    public static HashMap<Integer, Integer> mClickScore = new HashMap<>();
    private static HashMap<Integer, Integer> mScore = new HashMap<>();
    ...
    public int getScore(int type){
        return mScore.get(type);
    }

    public void setScore(int type, int score) {
        if(score == 0 && mScore.containsKey(type)){
            this.mScore.remove(type);
        }
        this.mScore.put(type,score);
    }
    ...
    public static class Water{
        ...
        Water(int key, int id, int period, int penaltyTime, int penalty, int neddWaterNum){
            waterKey = key;
            waterId = id;
            waterPeriod  = period;
    ...
    /*
    public void flowerLevelUp(DB_Exception.Flower flower){
    ...
    private void newCost(DB_Exception.Flower flower){
